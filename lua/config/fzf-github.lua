        ...
        ['default'] = function(selected)
                ...
                local view_opts = {
                    ...
                    actions = {
                        ['default'] = function(pr_selected)
                            local cmd = string.format("silent !gh pr view %s --web", get_pr_number(pr_selected[1]))
                            vim.cmd(cmd)
                        end
                    }
                }
            ...
            elseif selected[1] == "pr checkout" then
                local checkout_opts = {
                    ...
                    actions = {
                        ['default'] = function(pr_selected)
                            local cmd = string.format("!gh pr checkout %s", get_pr_number(pr_selected[1]))
                            vim.cmd(cmd)
                        end
                    }
                }
            ...
            elseif selected[1] == "browse file" then
                local cmd = string.format("silent !gh browse %s", get_current_file_line())
                vim.cmd(cmd)
            end
        end
    }

    fzf_lua.fzf_exec({ "pr checkout", "pr view", "browse file", "repo view" }, opts)
end
