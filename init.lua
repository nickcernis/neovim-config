--  ╭──────────────────────────────────╮
--  │ Hi, this is Nick's Neovim config │
--  ╰──────────────────────────────────╯
--    \
--     \
--  ⠀⠀⢀⣀⠤⠿⢤⢖⡆⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
...
-- Project switcher with zoxide
set("n", "<leader>fz", function()
  local home = vim.env.HOME
  local zoxide_list = vim.fn.systemlist("zoxide query -l")

  -- Format paths to replace home directory with ~
  local formatted_list = {}
  for _, path in ipairs(zoxide_list) do
    local display_path = path:gsub("^" .. home, "~")
    table.insert(formatted_list, display_path)
  end
  ...
  require("fzf-lua").fzf_exec(formatted_list, {
    ...
    actions = {
      ["default"] = function(selected)
        ...
        -- Notify terminal of directory change (OSC 7)
        local hostname = vim.fn.hostname()
        io.write(string.format("\027]7;file://%s%s\027\\", hostname, path))
        io.flush()
